...
def preprocess(x, reverse=False, dequantize=True, num_colors=256, alpha=0.05):
    if reverse:
        x = 1.0 / (1 + torch.exp(-x))
        x -= alpha
        x /= (1 - alpha)
        ...
        x = alpha + (1 - alpha) * x / num_colors
        logit = torch.log(x) - torch.log(1.0 - x)
        # determinant of transformation
        log_det = F.softplus(logit) + F.softplus(-logit) + torch.log(torch.tensor(1 - alpha)) - torch.log(torch.tensor(num_colors))
