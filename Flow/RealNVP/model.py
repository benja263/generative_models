...
class RealNVPScale(nn.Module):
    def __init__(self, scale_idx, num_scales, image_shape, n_res_blocks, num_filters):
        ...
        if self.last_scale:
            self.checker_board.append(AffineCheckerboardTransform(image_shape, 'odd', n_res_blocks, num_filters))
        else:
            ...
            self.next_scale = RealNVPScale(scale_idx+1, num_scales, (2*C, H // 2, W // 2), n_res_blocks, num_filters)
